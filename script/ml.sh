...
hadoop jar $jardir'mlctr-2.0.jar' com.elex.ssp.mlctr.Scheduler 0 5 >>$workdir'train.log' 2>&1
rm -rf $datadir'*.txt'
...
if [ $size -gt 10 ];then
   ...
   echo $ratio
   java -cp .:$jardir'mlctr-2.0.jar' com.elex.ssp.mlctr.liblinear.GJFormatSample $ratio $datadir'train_id.txt' $datadir'train_sam.txt'
