...
public class VectorizeReducer extends Reducer<Text, Text, Text, Text> {
	...
	private Map<String,String> odp_cluster_map = new HashMap<String,String>();
		...
		while (line != null) {
			...
			if (vList.length == 2) {
				if(vList[0].startsWith(FeaturePrefix.odp.getsName()+"_") || vList[0].startsWith(FeaturePrefix.cluster.getsName()+"_")){
					odp_cluster_map.put(vList[1], vList[0]);
				}else{
	...
	protected void reduce(Text key, Iterable<Text> values, Context context)
		...
		for (Text v : values) {
			...
			if (kv.length == 13) {
				...
				//add other feature,由于同组特征可能有多条记录，因此用map
		...
		if(userInfoMap.get(u_name) != null){
			...
			try {
                ...
                if(result.get("cl")!= null ){					
					...
					list.add(new Feature(odp_cluster_map.get(result.get("cl")),result.get("cl"),"1"));																				
					
				}
                
                if(result.get("odp")!= null ){					
					
					list.add(new Feature(odp_cluster_map.get(result.get("odp")),result.get("odp"),"1"));																				
